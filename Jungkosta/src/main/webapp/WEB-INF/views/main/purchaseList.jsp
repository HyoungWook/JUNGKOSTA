	...
	$(function() {
		...
		$('form').submit(
				function(event) {
					...
					var sendData = "purchase_id=" + purchase_id + "&sale_id="
							+ sale_id;
		...
		$('.payProc')
				.on(
						...
						function() {
							...
							var input = "<input type='hidden' name='purchase_id' value='"
									+ purchaseid.val() + "'>";
							var input2 = "<input type='hidden' name='sale_id' value='"
									+ saleid.val() + "'>";
					...
					<tbody>
						<c:forEach items="${list}" var="item">
							<tr>
								...
								<td><a
									href="/Jungkosta/trade/tradeDetail?sale_id=${item.sale_id}">${item.item_name}</a></td>
								<td>${item.email}</td>
								<td>${item.purchase_cost }</td>
								<td>${item.purchase_deadline }</td>

								<td>
									<%-- <button class="btn btn-primary" value="무통장 입금" id="${item.sale_id}">무통장 입금</button> --%>
									<button type="button" class="btn btn-primary payProc"
										data-target="#myModal" data-toggle="modal">무통장입금</button> <input
									type="hidden" class="purchase" name="purchase_id"
									value="${item.purchase_id}"> <input type="hidden"
									class="sale" name="sale_id" value="${item.sale_id}"> <input
									type="hidden" id="defaultcost" name="purchase_cost"
							...
							<div class="tw_form">
								<table class="transfer">
									...
									<tr>
										...
										<td align="left"><input type="text" name="payment_cost"
											class="purchase_cost"></td>
