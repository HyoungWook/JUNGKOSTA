...
<link href="../bootstrap/css/custom2.css" rel"src/main/webapp/WEB-INF/views/main/messageSendForm.jsp"="stylesheet"> -->
...
<script type="text/javascript">
	...
	function sendMessage() {
		...
		var $message = $('#ms_reciever_ws').val() + "/"
				+ $('#ms_sender_ws').val() + "/" + $('#ms_title_ws').val()
				+ "/" + $('#ms_content_ws').val();
...
<body>
	<div class="container">
		<div class="row">
			...
			<hr>
			<form id="ms_form_ws" class="form-horizontal" role="form"
				method="post" action="messageSendProc">
				<div class="form-group">
					<label for="reciever_ws" class="col-xs-2 col-lg-2 control-label">받는
						사람</label>
					<div class="col-xs-10 col-lg-10">
						<c:choose>
							<c:when test="${param.flag == '1' }">
								<input type="email" class="form-control" placeholder="받는 사람"
									id="ms_reciever_ws" name="receiver" value="admin"
									readonly="readonly">
							</c:when>
							<c:otherwise>
								<input type="email" class="form-control" placeholder="받는 사람"
									id="ms_reciever_ws" name="receiver">
							</c:otherwise>
						</c:choose>
					</div>
				</div>
				<div class="form-group">
					<label for="sender_ws" class="col-xs-2 col-lg-2 control-label">보내는
						사람</label>
					<div class="col-xs-10 col-lg-10">

						<c:choose>
							<c:when test="${param.flag == '1' }">
								<input type="email" class="form-control" placeholder="보내는 사람"
									id="ms_sender_ws" name="sender">
							</c:when>
							<c:otherwise>
								<input type="email" class="form-control" placeholder="보내는 사람"
									value="${email }" readonly="readonly" id="ms_sender_ws"
									name="sender">
							</c:otherwise>
						</c:choose>
					</div>
